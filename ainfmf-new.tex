...
In this paper we study $A_\infty$-categories of matrix factorizations, obtained from the natural DG-category of matrix factorizations by the minimal model construction. Our point of view diverges from the existing mathematical literature in that we genuinely seek to understand the full $A_\infty$-category and not just the minimal model of the endomorphism DG-algebra of a generator. Our motivation for pursuing this more general, and harder, question is twofold. Firstly, we believe this is the appropriate framework for actually doing calculations in the context of deformations of singularities and of matrix factorizations, as it is often inconvenient to explicitly present a given matrix factorization as a twisted complex over the generator. Secondly, there is interesting work in this generality in the physics literature, which deserve further study.
...
\]
where $F_\theta$ is the $\nZ_2$-graded $k$-module $\oplus_{i=1}^n k \theta_i$ which is concentrated in odd degree.

It turns out that while there is no obvious strict deformation retract on $\md{C}$, the DG-category $\md{C}_\theta$ can be equipped with a natural strict deformation retract which arises from differentiating in the normal directions to the critical locus (since the critical locus may not be reduced, this is admittedly a strange form of differentiation). The strict deformation retract puts $\md{C}_\theta$ in homotopy equivalence with the tensor product
\be\label{eq:tensor_introa}
\md{C} \otimes_R \jac
\ee
viewed as a collection of $\nZ_2$-graded complexes of finitely-generated free $k$-modules, and the usual $A_\infty$-minimal model construction equips this collection of graded modules with an $A_\infty$-structure $\{m_n\}_n$, the $m_1$ and $m_2$ of which are the differential and product that is given by interpreting \eqref{eq:tensor_introa} as a tensor product of DG-categories, with $\jac$ given the zero differential and its usual algebra structure.

Not all minimal models are created equal, since without a good grasp on the combinatorics of the homotopy which is part of the strict deformation retract, it is hopeless to try and understand the higher products which are produced. However, the strict deformation retract involved in the above construction is of a standard kind, so that the higher products may be described by a small collection of local ``Feynman rules''. 
...
Calculating beyond this point involves, for say $k$ a field, Gaussian eliminations and thus regular functions of the input data - the output and nature of the cohomology classes varies across strata defined by the vanishing of the denominators in such functions. In particular, the cohomology $H^*( \md{C} \otimes_R \jac )$, the induced $A_\infty$ structure on this cohomology, and the splitting of the induced (strictly) idempotent $A_\infty$-functor $H^*(\widetilde{e})$ cannot typically be presented by universal formulas which are polynomial in the entries of the twisted differentials. 
