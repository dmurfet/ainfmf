...
In the definition of the Feynman rules we considered operators on the $\nZ_2$-graded $k$-module $\mathscr{H}$ of Definition \ref{defn:handop}. But we observed in Remark \ref{??} that due to the nature of the operators involved, given an initial state $\Psi \in \mathscr{B}$ the value of $\cat{O}(T,C)$ on $\Psi$ for any tree $T$ and configuration $C$ belongs to $\mathscr{B}$.

\label{defn:handop} Consider the $\nZ_2$-graded algebra defined by the tensor product
\be
\mathscr{H} = R \otimes_k \bigwedge\big( \oplus_{i=1}^n k \theta_i \,\big) \otimes_k \End_k\Big( \bigwedge\big( \oplus_{i=1}^n k \psi_i \, \big) \Big)
\ee
