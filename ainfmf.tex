...
\end{center}
The amplitude of this Feynman diagram is the value of
\begin{align*}
...
t(e) = 1\,.
\end{gather*}
At $i_3$ we have the pair $a = 1$ and $x^\gamma = x^2$. Consequently $N_C(e) = 1$.
\end{example}
...
\begin{example} Let $W = x^d$ with $d > 2$. Then the $A_\infty$-products on the minimal model
\[
\mathscr{B} = \Big( \bigwedge \big( k \psi^* \big), \{ \rho_q \}_{q \ge 2} \; \Big)
\]
are given by
\begin{itemize}
\item $\rho_2$ is the forward suspension of the usual product in the exterior algebra, that is,
\[
\rho_2( x_1 \otimes x_2 ) = (-1)^{\widetilde{x_1} \widetilde{x_2} + \widetilde{x_1} + 1} x_2 x_1\,.
\]
\item For $q \notin \{2,d\}$ we have $\rho_q = 0$, and
\item The tensor product $\mathscr{B}^{\otimes d}$ has dimension $2^d$ with basis $(\psi^*)^{i_0} \otimes \cdots \otimes (\psi^*)^{i_d}$ for $i_j \in \{0,1\}$. The operator $\rho_d$ is zero on all the basis elements except for
\[
\rho_d( \psi^* \otimes \psi^* \otimes \cdots \otimes \psi^* ) = (-1)^{d-1}\,.
\]
\end{itemize}
\end{example}
